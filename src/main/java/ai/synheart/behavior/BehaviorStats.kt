...
data class BehaviorStats(
        /** Current typing cadence (keys per second). (Removed - typing tracking disabled) */
        val typingCadence: Double? = null,

        /** Current inter-key latency in milliseconds. (Removed - typing tracking disabled) */
        val interKeyLatency: Double? = null,

        /**
         * Current burst length (number of keys in current burst). (Removed - typing tracking
         * disabled)
         */
    ...
    init {
        require(appSwitchesPerMinute >= 0) {
            "appSwitchesPerMinute must be non-negative, got: $appSwitchesPerMinute"
        }
        stabilityIndex?.let {
    ...
    fun toMap(): Map<String, Any?> {
        return mapOf(
                // Typing fields removed - typing tracking disabled
